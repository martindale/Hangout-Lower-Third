(function(){
	 ...
	 * 
	 * Lower Third for Google+ Hangouts is distributed in the hope that it will be useful,
	...
	*/
	function ApplicationController(){
		...
		*/
		...
		/**
		 * @ApllicationController.globalShowCustom - defines the initial state of globalShow 
		 * @private
		 * @type {boolean}
		*/
		...
		*/
		...
		/**
		 * @ApllicationController.canvas_screen - defines the fullscreen canvas container 
		 * @private
		 * @type {null | HTMLCanvasElement}
		*/
	...
	*/
	ApplicationController.prototype.buildDOM = function(){
		...
		*/
		...
		footer.append(this.createElement("span", {"class":"version"}).text("v 1.0.0"));
		...
		*/
		...
		fieldrow_custom.append(label_custom, inputFile_custom);

		fieldset.append(fieldrow_name,fieldrow_tagline,fieldrow_select,fieldrow_logo,fieldrow_custom);
	...
	*/
	 ...
	 * @private
	*/
	ApplicationController.prototype.createClock = function(){
	 ...
	 * @param canvas {HTMLCanvasElement}
	*/
	ApplicationController.prototype.prepareCanvasContext = function(canvas,h,w){
	...
	ApplicationController.prototype.createCanvas = function(){
		...
		*/
		...
		var finish = function(){
			...
			*/
			this.canvasOverlays['canvasOverlay'] = canvasImage.createOverlay({
			});

			/*
			 * Set face tracking overlay parameters and toggle between Show/Hide
			*/
			this.canvasOverlays['canvasOverlay'].setScale(1, gapi.hangout.av.effects.ScaleReference.WIDTH);
			this.canvasOverlays['canvasOverlay'].setPosition(0, 0.39);
	...
	*/
	...
	ApplicationController.prototype.toggleShowCustom = function(){
		/*
		 * validate input data
		*/
		...
		if(this.globalShowCustom === false){
			...
			this.readImageFromInput(document.getElementById("customfile"), function(data){
				...
				img.onload = function(){
					...
					this.customOverlay['overlay'] = this.overlayResource.createOverlay({});
					this.customOverlay['overlay'].setPosition(0, 0);
					//img.width/640 = scale
					if(img.height > img.width){
						var scale = img.height/360;
						if(scale >= 1){scale = 1}
						this.customOverlay['overlay'].setScale(scale, gapi.hangout.av.effects.ScaleReference.HEIGHT);
					}else{
						var scale = img.width/640;
						if(scale >= 1){scale = 1}
						this.customOverlay['overlay'].setScale(scale, gapi.hangout.av.effects.ScaleReference.WIDTH);
					}
					console.log(scale);
					this.customOverlay['overlay'].setVisible(true);
		...
		this.globalShowCustom = false;
		for(var index in this.customOverlay){
			this.customOverlay[index].setVisible(false);
			delete this.customOverlay[index];
